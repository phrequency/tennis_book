            ...
            unless d[:opponent] == ""
              ...
              unless Match.where(:player1_id => self.id, :player2_id => player.id, :score => d[:score]).first || Match.where(:player1_id => player.id, :player2_id => self.id, :score => d[:score]).first
                if t_name.include? "d)"
                  ...
                  Match.create(:player1_id => self.id, :player2_id => player.id, :result => d[:result], :score => d[:score], :doubles => d[:doubles], :partner => t_partner, :name => t_name, :link => t_link, :date => t_date)
                else
                  Match.create(:player1_id => self.id, :player2_id => player.id, :result => d[:result], :score => d[:score], :name => t_name, :link => t_link, :date => t_date)
                end
              end
            end
          end
